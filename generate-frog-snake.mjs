...
function frogSVG() {
  return `
    ...
    <circle cx="1.1" cy="1.7" r="0.35" fill="#1b4d2b" opacity="0.6"/>
    <line class="fs-tongue" x1="0" y1="1.3" x2="9" y2="1.3" stroke="#ff5c7a" stroke-width="1.4" stroke-linecap="round" transform="scale(0.15,1)" opacity="0"/>
  `;
}
...
// Orders a subset of cells into a walkable route: always hop to the
// nearest remaining cell rather than a random one anywhere on the board.
// This is what makes the cats look like they're prowling/stalking instead
// of teleporting corner-to-corner.
function nearestNeighborRoute(points, cellCenter, rng) {
  const remaining = points.slice();
  const startIdx = Math.floor(rng() * remaining.length);
  const route = [remaining.splice(startIdx, 1)[0]];
  while (remaining.length) {
    const [lx, ly] = cellCenter(route[route.length - 1].w, route[route.length - 1].d);
    let bestI = 0;
    let bestD = Infinity;
    for (let i = 0; i < remaining.length; i++) {
      const [x, y] = cellCenter(remaining[i].w, remaining[i].d);
      const dx = x - lx;
      const dy = y - ly;
      const d = dx * dx + dy * dy;
      if (d < bestD) {
...
function renderSVG(grid, theme) {
  ...
  const frogFrames = [];
  const eatEvents = [];
  let frogX0, frogY0, frogColor0;
  frogPath.forEach((p, i) => {
    const pct = ((i / (N - 1)) * 100).toFixed(3);
    const [x, y] = cellCenter(p.w, p.d);
    eaten += p.count;
    const t = eaten / total;
    const fill = frogFill(t, theme);
    if (i === 0) {
      frogX0 = x;
      frogY0 = y;
      frogColor0 = fill;
    }
    frogFrames.push(`${pct}% { transform: translate(${x}px, ${y}px); color: ${fill}; }`);
    if (p.count > 0) {
  ...
  // --- Ghosts (cats): each prowls a SMALL, spatially-connected subset of
  // cells (nearest-neighbour ordered, not a random cell anywhere on the
  // 370-cell board) so consecutive hops are always short. Fewer stops,
  // near-even dwell times, and an eased hop keep the pace close to
  // a walk instead of a vibrate. No two cats share a route, start point,
  // or rhythm, so they still don't move in lockstep with each other.
  const STOPS_PER_CAT = Math.max(14, Math.min(24, Math.round(basePath.length * 0.07)));
  let ghostKeyframes = "";
  let ghostRules = "";
  const ghosts = catColors
    .map((color, i) => {
      const pool = shuffled(basePath, rng).slice(0, STOPS_PER_CAT);
      const ghostPath = nearestNeighborRoute(pool, cellCenter, rng);
      const weights = ghostPath.map(() => 0.9 + rng() * 0.3); // gentle, near-even pacing
      ...
      // Always a little slower than the frog (never faster) - a chaser
      // that's slower than what it's chasing reads as tension, not chaos.
      const ghostDuration = (DURATION * (1.1 + rng() * 0.35)).toFixed(2);
      const [gx0, gy0] = cellCenter(ghostPath[0].w, ghostPath[0].d);
      ghostKeyframes += `@keyframes ${animName} { ${frames.join("\n")} 100% { transform: translate(${gx0}px, ${gy0}px); } }\n`;
      // Negative delay starts each cat at a different point in its own
      // loop immediately, instead of all four bursting from cell zero.
      const delay = (-(rng() * ghostDuration)).toFixed(2);
      ghostRules += `.fs-cat-${i} { animation: ${animName} ${ghostDuration}s ease-in-out ${delay}s infinite; }\n`;
      return `<g class="fs-cat fs-cat-${i}" transform="translate(${gx0}, ${gy0})">${catGhostSVG(color)}</g>`;
    })
    .join("\n");
  ...
  ${dots}
  <g>${ghosts}</g>
  <g class="fs-frog" transform="translate(${frogX0}, ${frogY0})" style="color:${frogColor0}">${frogSVG()}</g>
</svg>`;
}
